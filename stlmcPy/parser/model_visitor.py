...
from stlmcPy.core.formula import *
...
from stlmcPy.goal.goal import StlGoal
from stlmcPy.model.model import StlMC
...
class ModelVisitor(modelVisitor):
    ...
    def visitStlMC(self, ctx: modelParser.StlMCContext):
        ...
        print(propDecl)
        return (StlMC(modesDecl, varsDecl, varvalDict, modeModuleDecl, init, (propDecl + self.newPropDecl),
                      self.formulaText), goals)
    ...
    def visitGoal_decl(self, ctx: modelParser.Goal_declContext):
        ...
        for i in range(len(ctx.formula())):
            ...
            wfwf = self.visit(ctx.formula()[i])
            # type of formula

            formulaList.append(StlGoal(wfwf))
        return formulaList
