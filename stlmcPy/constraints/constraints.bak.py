...
class Binary:
    ...
    def __repr__(self):
        return repr(self.__left) + " " + repr(self.__right)
...
class Variable(Leaf, Expr):
    ...
    @property
    def type(self):
        return repr(self.__type)

    def __repr__(self):
        return repr(self.__type) + " " + repr(self.__id)
...
# mode var real type
# cont real
# class ContVar(ExprVariable):
#     def __init__(self, interval, varId):
#         super().__init__(varId)
#         self.__type = "real"
#         self.__varId = varId
...
#         variable = {'bool': Bool, 'int': Int, 'real': Real}[self.__type](self.__varId)
#         consts = list()
