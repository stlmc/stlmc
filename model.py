...
class Thermostat:
    ...
    def reach(self, ts):
        k = len(ts) - 1
        [qf, qs, qt] = modeDeclare(3, k+1, Type.BOOL) 
...
#            print(s.to_smt2())
    ...
    def flow(self, qf, qs, qt, state, time, ODElist, k):
        startList = []
        endList = []
        for i in range(len(state)):
            startList.append(state[i].start[k])
            endList.append(state[i].end[k])
        toFFF  = Implies(And(qf == self.qOff, qs == self.qOff, qt == self.qOff), Integral(endList, startList, time, ODElist[0]))
        toFFO  = Implies(And(qf == self.qOff, qs == self.qOff, qt == self.qOn), Integral(endList, startList, time, ODElist[1]))
        toFOF  = Implies(And(qf == self.qOff, qs == self.qOn, qt == self.qOff), Integral(endList, startList, time, ODElist[2]))
        toFOO  = Implies(And(qf == self.qOff, qs == self.qOn, qt == self.qOn), Integral(endList, startList, time, ODElist[3])) 
        toOFF  = Implies(And(qf == self.qOn, qs == self.qOff, qt == self.qOff), Integral(endList, startList, time, ODElist[4]))
        toOFO  = Implies(And(qf == self.qOn, qs == self.qOff, qt == self.qOn), Integral(endList, startList, time, ODElist[5]))
        toOOF  = Implies(And(qf == self.qOn, qs == self.qOn, qt == self.qOff), Integral(endList, startList, time, ODElist[6]))
        toOOO  = Implies(And(qf == self.qOn, qs == self.qOn, qt == self.qOn), Integral(endList, startList, time, ODElist[7]))
