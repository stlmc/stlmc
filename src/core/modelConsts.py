...
#                modeConsts.append(And(Real('currentMode_'+str(k)) >= RealVal(i), Real('currentMode_'+str(k)) <= RealVal(i)))
    ...
    def dreal4PropForall(self, exp, bound, curFlow):
        d4h = dreal4Handler()
        const = d4h.dreal4Obj(exp)
        #print(type(const))
        #print(type(curFlow))
        #print(str(curFlow))
        #print(d4h.dreal4Obj(curFlow))
            ...
            for i in self.makePropDict().keys():
                if str(i) in propSet:
                    for m in range(len(self.modeModule)):
                        ...
                        if solver == "dreal4":
                            const.append(Implies(And(i, curMode).substitution(combine), self.dreal4PropForall(self.makePropDict()[i], k, curFlow)))
                            const.append(Implies(And(Not(i), curMode).substitution(combine), self.dreal4PropForall(Not(self.makePropDict()[i]).reduce(), k, curFlow)))
                        else:
