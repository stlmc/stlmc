...
from ...constraints.aux.operations import variable_equal
...
def _analysis(alg, clustering):
    return "sol = solve(prob, tspan=(0.0, 10.0), alg={}, clustering_method={})".format(alg, clustering)
...
def _unsafe_check(ha: HybridAutomaton, mode_id: Dict[Mode, int]):
    ...
    for mode in ha.modes:
        if mode.is_final():
            final_m_s.add(str(mode_id[mode]))

    return "\n".join(["ce_found = false",
                      "for fp in flowpipe(sol)",
                      "if location(fp) in [{}]".format(", ".join(final_m_s)),
                      "println(\"counterexample found\")",
                      "global ce_found = true", "break", "end", "end",
...
def _make_inv(mode: Mode):
    ...
    if len(mode.invariant) > 0:
        ...
        inv = ""
    return ("", "") if inv == "" else ("{} = {}".format(inv_name, inv), ", x ∈ {}".format(inv_name))
